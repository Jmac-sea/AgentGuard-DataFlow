...
export function BenchmarkPage({ benchmark, live }: { benchmark: Benchmark; live: boolean }) {
  return (
    <section className="panel benchmark-page">
      <div className="panel-heading"><div><span className="eyebrow">{benchmark.run_id}</span><h2>防护效果基准评测</h2><p>{benchmark.transport ?? "inprocess"} · 每种模式运行 {benchmark.runs_per_mode} 次</p></div><span className={`live-pill ${live ? "connected" : "demo"}`}>{live ? "LIVE DATA" : "DEMO DATA"}</span></div>
      <div className="benchmark-table">
        <div className="benchmark-row header"><span>模式</span><span>攻击成功率</span><span>秘密泄漏率</span><span>阻断率</span><span>P95耗时</span></div>
        {benchmark.metrics.map((metric) => (
          <div className="benchmark-row" key={metric.mode}>
            ...
            <span>{metric.p95_duration_ms.toFixed(1)}ms</span>
          </div>
        ))}
      </div>
